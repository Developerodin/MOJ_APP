...
import { bookmark, bookmarkOutline, chevronBackOutline, chevronDownOutline, documentTextOutline, eyeOffOutline, eyeOutline, filterOutline } from 'ionicons/icons'
...
export const ViewedJobs = () => {
  ...
  const [ViewedJobData,setViewedJobData] = useState([]);
  ...
  const getViewedJobs = async (jobData) => {
    try {
      const url = `${Base_url}job_view/Byuserid/${userDetails.user_id}`;
      const formData1 = new FormData();
      // formData1.append('user_id', userDetails.user_id);
      // formData1.append('resume', selectedFile);
...
<IonPage>
  ...
  <IonRow >
    {
      ViewedJobData && ViewedJobData.map((el,index)=>{
